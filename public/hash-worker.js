...
// Import streaming hash libraries for worker scope
try {
  importScripts(
  ...
  // If offline or bundled, scripts may already be attached or handled by the bundler
}
...
async function hashFileInWorker(
  ...
  // Zero-byte edge case per ISO/IEC 27037 baseline
  if (file.size === 0) {
    self.postMessage({
      type: 'CHUNK_PROGRESS',
  ...
  // Fallback slice chunking with immediate garbage collection
  if (!streamWorked) {
    let offset = 0;
    ...
    while (offset < totalSize) {
      ...
      // Explicit de-allocation to prevent heap bloat
      arrayBuffer = null;
      uint8 = null;
    }
  }
